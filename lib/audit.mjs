...
function decodeDocument(buffer, source) {
  let content = buffer;
  if (isGzip(content)) {
    try {
  ...
  if (content.length > MAX_XML_BYTES) {
    throw new Error(`${source} exceeds the 50 MiB uncompressed document limit`);
  }
...
function normalizeInitialSource(input) {
  ...
  if (parsed?.protocol === 'https:') return { kind: 'https', id: parsed.href };
...
function resolveChildSource(loc, parent) {
  ...
  if (parent.kind === 'https') {
    const resolved = new URL(loc, parent.id);
    if (resolved.protocol !== 'https:') {
      throw new Error(`Child sitemap resolved to non-HTTPS URL: ${resolved.href}`);
    }
    return { kind: 'https', id: resolved.href };
  }
...
async function loadSource(source) {
  if (source.kind === 'file') {
    let buffer;
    try {
      buffer = await readFile(source.id);
    } catch (error) {
      throw new Error(`Could not read ${source.id}: ${error.message}`);
    }
    return decodeDocument(buffer, source.id);
  }

  let response;
  try {
    response = await fetch(source.id, {
      headers: { 'user-agent': `sitemap-cohort-auditor/${VERSION}` },
      redirect: 'follow',
      signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS),
    });
  } catch (error) {
    throw new Error(`Could not fetch ${source.id}: ${error.message}`);
  }

  if (!response.ok) {
    throw new Error(`Could not fetch ${source.id}: HTTP ${response.status}`);
  }
  if (new URL(response.url).protocol !== 'https:') {
    throw new Error(`HTTPS source redirected to a non-HTTPS URL: ${response.url}`);
  }

  const length = Number(response.headers.get('content-length'));
  if (Number.isFinite(length) && length > MAX_XML_BYTES) {
    throw new Error(`${source.id} declares a response larger than 50 MiB`);
  }
...
async function walk(source, state) {
  ...
  const xml = await loadSource(source);
  const kind = documentKind(xml);
  state.documents.push({ source: source.id, type: kind });
  ...
  if (kind === 'sitemapindex') {
    ...
    for (const block of sitemapBlocks) {
      ...
      if (!loc) {
        state.missingLocs.push({ context: 'sitemap', source: source.id });
        continue;
      }
      await walk(resolveChildSource(loc, source), state);
    }
    return;
  }
  ...
  for (const block of extractBlocks(xml, 'url')) {
    ...
    if (!loc) {
      state.missingLocs.push({ context: 'url', source: source.id });
      continue;
    }
    recordUrl(state, loc, source.id);
  }
}
...
function formatNamedCounts(records) {
  return records.length > 0
    ? records.map(({ name, count }) => `${name}=${count}`).join(', ')
    : 'none';
}

export function formatTextReport(report) {
  const lines = [
    `Sitemap cohort audit: ${report.source}`,
    '',
  ...
  if (report.duplicates.length > 0) {
    ...
    for (const duplicate of report.duplicates) {
      lines.push(`  ${duplicate.count}x ${duplicate.url}`);
    }
  }
  ...
  if (report.invalidLastmods.length > 0) {
    ...
    for (const item of report.invalidLastmods) {
      lines.push(`  ${JSON.stringify(item.value)} (${item.url ?? item.source})`);
    }
  }

  if (report.fragments.length > 0) {
    lines.push('', 'URLs with fragments:');
    for (const item of report.fragments) lines.push(`  ${item.url}`);
  }
  ...
  if (report.comparison) {
    lines.push(
      ...
      ...report.comparison.added.map((url) => `  + ${url}`),
      `Removed: ${report.comparison.removedCount}`,
      ...report.comparison.removed.map((url) => `  - ${url}`),
    );
  }
